...
export function initializeGPT() {
  ...
  /**
   * Assertive Yield will load GPT.js
   */
  // appendResource(
  //   'script',
  ...
  function runRefreshEvent() {
    ...
    if (window.googletag && googletag.pubadsReady) {
      if (window.onDvtagReady) {
        // https://support.doubleverify.com/support/solutions/articles/22000284278-unity-tag-integration
        window.onDvtagReady(() => {
          window.googletag
            .pubads()
            .refresh([ad], { changeCorrelator: correlator });
        });
      } else {
        window.googletag
          .pubads()
